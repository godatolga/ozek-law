    ...
    <div style={{position:"fixed",inset:0,zIndex:500,display:"flex",flexDirection:"column"}}>
      <div className="modal-enter" style={{position:"relative",background:"#0D1628",flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 22px 8px"}}>
          ...
          <button onClick={onClose} style={{width:32,height:32,borderRadius:99,background:"rgba(255,255,255,0.12)",border:"none",color:"#FFF",fontSize:19,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
        </div>
        <div style={{overflow:"auto",padding:"4px 22px 28px"}}>
          {done?(
            <div style={{textAlign:"center",padding:"32px 0 16px"}}>
              ...
              <p style={{fontSize:14,color:"rgba(255,255,255,0.6)",lineHeight:1.6,marginBottom:24}}>{days.find(d=>d.num+d.month===selDay)?.date.toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})} at {selTime}</p>
              <button onClick={onClose} style={{background:"rgba(255,255,255,0.10)",color:"#FFF",border:"1px solid rgba(255,255,255,0.14)",borderRadius:14,padding:"13px 32px",fontSize:15,fontWeight:600,cursor:"pointer"}}>Done</button>
            </div>
          ):(
            <>
              <div style={{marginBottom:20}}>
                <div style={{fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:"#C9A84C",fontWeight:600,marginBottom:8}}>SCHEDULE</div>
                <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:22,color:"#FFF",letterSpacing:-0.4}}>Consultation</h2>
                <p style={{fontSize:13,color:"rgba(255,255,255,0.5)",marginTop:4}}>Choose a date and time · Atty. Tolga Ozek</p>
              </div>
              <div style={{fontSize:11,color:"rgba(255,255,255,0.4)",letterSpacing:1.2,textTransform:"uppercase",marginBottom:10}}>Select Date</div>
              <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4,marginBottom:20}}>
                {days.map((d,i)=>{const key=d.num+d.month;const sel=selDay===key;return(
                  <button key={i} onClick={()=>setSelDay(key)} style={{flexShrink:0,width:54,padding:"10px 0",borderRadius:16,background:sel?"#C9A84C":"rgba(255,255,255,0.06)",border:sel?"none":"1px solid rgba(255,255,255,0.09)",cursor:"pointer",textAlign:"center",transition:"all 0.18s",transform:sel?"scale(1.08)":"scale(1)"}}>
                    <div style={{fontSize:10,color:sel?"rgba(15,25,35,0.7)":"rgba(255,255,255,0.4)",marginBottom:3,fontWeight:600,textTransform:"uppercase"}}>{d.label}</div>
                    <div style={{fontSize:19,fontWeight:800,color:sel?"#0F1923":"#FFF",fontFamily:"'Playfair Display',serif"}}>{d.num}</div>
                    <div style={{fontSize:10,color:sel?"rgba(15,25,35,0.6)":"rgba(255,255,255,0.3)",marginTop:2}}>{d.month}</div>
                  </button>
                );})}
              </div>
              <div style={{fontSize:11,color:"rgba(255,255,255,0.4)",letterSpacing:1.2,textTransform:"uppercase",marginBottom:10}}>Select Time</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:22}}>
                {slots.map(s=>{const sel=selTime===s;return(
                  <button key={s} onClick={()=>setSelTime(s)} style={{padding:"12px 0",borderRadius:14,background:sel?"#C9A84C":"rgba(255,255,255,0.06)",border:sel?"none":"1px solid rgba(255,255,255,0.09)",color:sel?"#0F1923":"#FFF",fontSize:14,fontWeight:600,cursor:"pointer",transition:"all 0.18s",transform:sel?"scale(1.04)":"scale(1)"}}>{s}</button>
                );})}
              </div>
              <button onClick={confirm} disabled={!selDay||!selTime} style={{width:"100%",background:selDay&&selTime?"#C9A84C":"rgba(255,255,255,0.08)",color:selDay&&selTime?"#0F1923":"rgba(255,255,255,0.3)",border:"none",borderRadius:16,padding:"16px",fontSize:16,fontWeight:700,cursor:selDay&&selTime?"pointer":"not-allowed",transition:"all 0.2s"}}>
      ...
      <div style={{display:"flex",gap:14,alignItems:"center",cursor:"pointer"}} onClick={()=>setExpanded(e=>!e)}>
        <div style={{position:"relative",flexShrink:0,width:90,height:90}}>
          ...
          <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
            <div style={{fontSize:19,fontWeight:800,color:"#C9A84C",fontFamily:"'Playfair Display',serif",lineHeight:1}}>{pct}%</div>
            <div style={{fontSize:8,color:"#8492A6",marginTop:2}}>DONE</div>
          </div>
        </div>
        <div style={{flex:1}}>
          <div style={{fontSize:10,letterSpacing:2,textTransform:"uppercase",color:"#C9A84C",fontWeight:600,marginBottom:4}}>CASE PROGRESS</div>
          <div style={{fontSize:14,fontWeight:700,color:"#0F1923",marginBottom:3}}>{client.matters[0]}</div>
          <div style={{fontSize:12,color:"#8492A6",marginBottom:6}}>{done} of {TIMELINE.length} milestones</div>
          <div style={{background:"rgba(201,168,76,0.10)",border:"1px solid rgba(201,168,76,0.22)",borderRadius:10,padding:"5px 10px",fontSize:11,color:"#9A7A2E",fontWeight:600,display:"inline-block"}}>Next: USCIS Receipt</div>
        </div>
        <div style={{color:"#C9A84C",fontSize:13,transform:expanded?"rotate(180deg)":"rotate(0deg)",transition:"transform 0.2s",flexShrink:0}}>▾</div>
      </div>
      {expanded&&(
        <div className="pop-in">
          ...
          {TIMELINE.map((s,i)=>(
            <div key={i} style={{display:"flex",gap:14,alignItems:"flex-start",marginBottom:i<TIMELINE.length-1?14:0}}>
              <div style={{width:28,height:28,borderRadius:"50%",flexShrink:0,background:s.done?"#C9A84C":s.active?"rgba(201,168,76,0.15)":"rgba(0,0,0,0.05)",border:s.active?"2px solid #C9A84C":"2px solid transparent",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>
                {s.done?"✓":s.icon}
              </div>
              <div style={{paddingTop:3}}>
                <div style={{fontSize:13,fontWeight:s.active||s.done?700:500,color:s.done?"#0F1923":s.active?"#9A7A2E":"#8492A6"}}>{s.step}</div>
                <div style={{fontSize:11,color:"#8492A6"}}>{s.date}</div>
                {s.active&&<span style={{background:"rgba(201,168,76,0.12)",color:"#9A7A2E",fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:99,marginTop:4,display:"inline-block"}}>IN PROGRESS</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {[["#30D158","Paid",paid],["#FF453A","Due",due],["#0A84FF","Upcoming",soon]].map(([color,label,amt])=>(
          <div key={label} style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            <div><div style={{fontSize:11,color:"#8492A6"}}>{label}</div><div style={{fontSize:16,fontWeight:800,color:"#0F1923",fontFamily:"'Playfair Display',serif"}}>${amt.toLocaleString()}</div></div>
          </div>
        ))}
      </div>
    </div>
...
function LangSwitcher({lang,setLang}){
  ...
  return(
    <div style={{position:"relative",zIndex:200}}>
      <button onClick={()=>setOpen(o=>!o)} style={{background:"rgba(201,168,76,0.15)",backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",border:"1px solid rgba(201,168,76,0.30)",borderRadius:99,padding:"5px 13px",fontSize:12,fontWeight:700,color:C.gold,display:"flex",alignItems:"center",gap:5}}>
        ...
        <div style={{position:"fixed",top:64,right:16,background:"rgba(10,18,35,0.97)",backdropFilter:"blur(32px)",WebkitBackdropFilter:"blur(32px)",border:"1px solid rgba(255,255,255,0.10)",borderRadius:18,overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,0.5)",minWidth:160,zIndex:99999}}>
          {Object.values(LANGS).map(l=>(
            <button key={l.code} onClick={()=>{setLang(l.code);setOpen(false);}} style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"11px 16px",background:l.code===lang?"rgba(201,168,76,0.14)":"transparent",borderBottom:"1px solid rgba(255,255,255,0.06)",textAlign:"left"}}>
              <span style={{fontSize:12,fontWeight:700,color:C.gold,minWidth:28}}>{l.label}</span>
              <span style={{fontSize:13,color:l.code===lang?C.hi:C.md}}>{l.name}</span>
            </button>
          ))}
        </div>
      )}
  ...
  return(
    <div id="top-bar" style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"6px 20px",background:"rgba(8,15,30,0.0)",borderBottom:"none",flexShrink:0,position:"fixed",top:0,left:0,right:0,zIndex:1000,willChange:"transform",transition:"transform 0.22s cubic-bezier(0.4,0,0.2,1)"}}>
      <div style={{width:60,display:"flex",justifyContent:"flex-start"}}>
        {client&&<button onClick={()=>{setClient(null);setTab("home");}} style={{fontSize:12,color:"rgba(255,255,255,0.8)",fontWeight:500,background:"rgba(0,0,0,0.3)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:99,padding:"5px 10px"}}>{t.signOut}</button>}
      </div>
        ...
        return(
          <button key={tb.id} onClick={()=>setTab(tb.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"2px 0"}}>
            <div style={{width:36,height:28,borderRadius:10,background:active?"rgba(201,168,76,0.18)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s",transform:active?"scale(1.08)":"scale(1)"}}>
              <span style={{fontSize:19,opacity:active?1:0.3}}>{tb.icon}</span>
            </div>
            <span style={{fontSize:10,fontWeight:active?700:400,color:active?C.gold:"#FFFFFF",transition:"all 0.2s"}}>{tb.label}</span>
          </button>
        );
      })}
    </nav>
    ...
    <div dir={dir}>
      <div style={{position:"relative",overflow:"hidden",minHeight:"52vh",display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
        ...
        <div style={{position:"relative",padding:"28px 22px 32px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:32}}>
            <div style={{fontSize:10,color:C.lo,letterSpacing:2,textTransform:"uppercase"}}>{t.tagline}</div>
            <button onClick={()=>setTab("portal")} style={{background:C.gold,color:"#0F1923",border:"none",borderRadius:12,padding:"9px 18px",fontSize:13,fontWeight:700,cursor:"pointer",flexShrink:0}}>{t.clientLogin}</button>
          </div>
          <div style={{marginBottom:24}}>
            ...
            <p style={{fontSize:14,color:C.md,lineHeight:1.72,fontWeight:300,maxWidth:340}}>{t.heroSub}</p>
          </div>
          <div style={{display:"flex",gap:10,marginBottom:32}}>
            <button onClick={()=>window.open("https://outlook.office.com/book/OzeklawTolgaOzekConsultations@NETORGFT7968746.onmicrosoft.com/?ismsaljsauthenabled","_blank")} style={{flex:3,background:C.gold,color:"#0F1923",border:"none",borderRadius:14,padding:"14px 0",fontSize:15,fontWeight:700,cursor:"pointer"}}>{t.scheduleNow} ↗</button>
            <button onClick={()=>setShowBooking(true)} style={{flex:2,background:"rgba(255,255,255,0.12)",backdropFilter:"blur(20px) saturate(180%)",WebkitBackdropFilter:"blur(20px) saturate(180%)",color:"rgba(255,255,255,0.90)",border:"1px solid rgba(255,255,255,0.22)",boxShadow:"inset 0 1px 0 rgba(255,255,255,0.14),0 4px 16px rgba(0,0,0,0.15)",borderRadius:14,padding:"14px 0",fontSize:13,fontWeight:500,cursor:"pointer"}}>📅 Book Slot</button>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",background:"rgba(255,255,255,0.08)",backdropFilter:"blur(28px) saturate(180%)",WebkitBackdropFilter:"blur(28px) saturate(180%)",borderTop:"1px solid rgba(255,255,255,0.18)",boxShadow:"inset 0 1px 0 rgba(255,255,255,0.12)",padding:"14px 0"}}>
          {[["98%","Approval"],["15+","Years"],["20+","Areas"],["42","Countries"]].map(([n,l],i)=>(
            <div key={l} style={{textAlign:"center",borderRight:i<3?"1px solid rgba(255,255,255,0.07)":"none"}}>
              ...
              <div style={{fontSize:10,color:C.lo,marginTop:3,letterSpacing:0.5,textTransform:"uppercase"}}>{l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{padding:"28px 22px 28px",background:"#FFFFFF"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:16}}>
          <div>
            <div style={{fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:C.gold,fontWeight:600,marginBottom:5}}>EXPERTISE</div>
            <h2 style={{fontSize:20,fontWeight:700,color:C.ink,fontFamily:"'Playfair Display',serif",letterSpacing:-0.3}}>{t.practiceAreas}</h2>
          </div>
          <button onClick={()=>window.open("https://ozeklaw.com/practiceareas/","_blank")} style={{fontSize:13,color:C.gold,fontWeight:600,background:"none",border:"none",cursor:"pointer"}}>{t.seeAll}</button>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:0}}>
          {t.practiceList.map((p,i)=>(
            <div key={p.title} onClick={()=>window.open("https://ozeklaw.com/practiceareas/","_blank")} style={{background:i%2===0?"#F7F5F2":"rgba(201,168,76,0.08)",border:i%2===0?"1px solid rgba(0,0,0,0.06)":"1px solid rgba(201,168,76,0.20)",boxShadow:"0 2px 10px rgba(0,0,0,0.05)",borderRadius:22,padding:"18px 16px 16px",cursor:"pointer",position:"relative",overflow:"hidden"}}>
              ...
              <div style={{fontSize:12,color:C.inkMd}}>{p.sub}</div>
              <div style={{position:"absolute",bottom:12,right:14,color:C.gold,fontSize:16,opacity:0.7}}>›</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{padding:"0 22px",background:"#FFFFFF"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:16}}>
          <div>
            <div style={{fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:C.gold,fontWeight:600,marginBottom:5}}>INSIGHTS</div>
      ...
      <div style={{padding:"28px 22px 28px",background:"#FFFFFF"}}>
        <div style={{background:"rgba(201,168,76,0.13)",border:"1px solid rgba(201,168,76,0.32)",boxShadow:"inset 0 1px 0 rgba(255,220,100,0.20)",borderRadius:24,padding:"24px 22px",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{position:"relative"}}>
            ...
            <p style={{fontSize:14,color:C.inkMd,marginBottom:18,lineHeight:1.6,fontWeight:300}}>{t.heroSub.slice(0,90)}…</p>
            <button onClick={()=>window.open("https://outlook.office.com/book/OzeklawTolgaOzekConsultations@NETORGFT7968746.onmicrosoft.com/?ismsaljsauthenabled","_blank")} style={{width:"100%",background:C.gold,color:"#0F1923",border:"none",borderRadius:14,padding:"15px",fontSize:15,fontWeight:700,cursor:"pointer"}}>{t.scheduleNow} ↗</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return(
    <div dir={dir} style={{background:"#F7F5F2",minHeight:"100%"}}>
      <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",padding:"22px 22px 0",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative"}}>
          <div style={{fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:C.gold,fontWeight:600,marginBottom:8}}>UPDATES</div>
          ...
          <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:16,scrollbarWidth:"none"}}>
            {[["news","📰","News"],["visa","📅","Visa Bulletin"],["instagram","📸","Instagram"],["linkedin","💼","LinkedIn"],["regs","🏛️","Rules & Regs"]].map(([id,icon,label])=>(
              <button key={id} onClick={()=>setNewsTab(id)} style={{padding:"7px 14px",borderRadius:99,fontSize:12,fontWeight:600,cursor:"pointer",border:"none",background:newsTab===id?C.gold:"rgba(255,255,255,0.12)",color:newsTab===id?"#0F1923":"rgba(255,255,255,0.75)",transition:"all 0.2s",flexShrink:0,whiteSpace:"nowrap"}}>{icon} {label}</button>
            ))}
          </div>
        </div>
      </div>
        ...
        <div style={{padding:"20px 18px 100px"}}>
          <div style={{background:"#FFFFFF",borderRadius:20,padding:"18px",marginBottom:18,border:"1px solid rgba(0,0,0,0.05)"}}>
            <div style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:12}}>
              <div style={{width:38,height:38,borderRadius:12,background:"rgba(201,168,76,0.12)",border:"1px solid rgba(201,168,76,0.20)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:19,flexShrink:0}}>📬</div>
              <div><div style={{fontSize:14,fontWeight:700,color:C.ink}}>{t.newsletter}</div><div style={{fontSize:12,color:C.inkMd,marginTop:2}}>{t.newsletterSub}</div></div>
            </div>
            {done?<div style={{background:"rgba(48,209,88,0.10)",borderRadius:12,padding:"11px 14px",color:C.green,fontSize:14,fontWeight:600}}>{t.subscribed}</div>:(
              <div style={{display:"flex",gap:8}}>
                <input value={sub} onChange={e=>setSub(e.target.value)} placeholder={t.emailPlaceholder} style={{flex:1,background:"rgba(0,0,0,0.04)",border:"1.5px solid rgba(0,0,0,0.07)",borderRadius:12,padding:"11px 14px",fontSize:14,color:C.ink,outline:"none"}}/>
        ...
        <div style={{padding:"20px 18px 100px"}}>
          <div style={{background:"linear-gradient(135deg,rgba(201,168,76,0.15),rgba(201,168,76,0.05))",border:"1px solid rgba(201,168,76,0.30)",borderRadius:20,padding:"16px",marginBottom:18}}>
            <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:8}}>
              <div style={{width:36,height:36,borderRadius:10,background:"rgba(201,168,76,0.20)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:19,flexShrink:0}}>📅</div>
              <div><div style={{fontSize:14,fontWeight:700,color:C.ink}}>April 2026 Visa Bulletin</div><div style={{fontSize:12,color:C.inkLo}}>Dates for Filing Chart (USCIS authorized)</div></div>
            </div>
            <div style={{fontSize:12,color:C.inkMd,lineHeight:1.5}}>Priority dates show when you can file. <strong style={{color:C.goldDim}}>"Current"</strong> = file now.</div>
          </div>
          <div style={{fontSize:14,fontWeight:700,color:C.ink,marginBottom:10}}>Family-Sponsored</div>
          {VB_FAM.map(r=><VBRow key={r.cat} r={r}/>)}
          <div style={{fontSize:14,fontWeight:700,color:C.ink,margin:"16px 0 10px"}}>Employment-Based</div>
          ...
          <div style={{background:"rgba(10,132,255,0.08)",border:"1px solid rgba(10,132,255,0.20)",borderRadius:16,padding:"14px",marginTop:12}}>
            <div style={{fontSize:12,color:C.inkLo,marginBottom:10}}>* Dates vary by country. China & India have earlier cutoffs. See USCIS for country-specific dates.</div>
            <div style={{fontSize:13,fontWeight:700,color:C.blue,marginBottom:6}}>📌 When to File</div>
            <div style={{fontSize:12,color:C.inkMd,lineHeight:1.6,marginBottom:10}}>File your I-485 when your priority date is earlier than the "File Date" above. Contact Ozek Law to confirm eligibility.</div>
            <div style={{display:"flex",gap:8}}>
              <a href="https://www.uscis.gov/green-card/green-card-processes-and-procedures/visa-availability-priority-dates/when-to-file-your-adjustment-of-status-application-for-family-sponsored-or-employment-based-123" target="_blank" rel="noreferrer" style={{flex:1,textDecoration:"none"}}><div style={{background:"rgba(10,132,255,0.12)",borderRadius:10,padding:"9px",fontSize:11,fontWeight:600,color:C.blue,textAlign:"center"}}>Family Chart ↗</div></a>
              <a href="https://www.uscis.gov/green-card/green-card-processes-and-procedures/visa-availability-priority-dates/when-to-file-your-adjustment-of-status-application-for-family-sponsored-or-employment-based-122" target="_blank" rel="noreferrer" style={{flex:1,textDecoration:"none"}}><div style={{background:"rgba(10,132,255,0.12)",borderRadius:10,padding:"9px",fontSize:11,fontWeight:600,color:C.blue,textAlign:"center"}}>Employment Chart ↗</div></a>
            </div>
          </div>
        </div>
      )}
      {newsTab==="instagram"&&(
        <div style={{padding:"20px 18px 100px"}}>
          <div style={{background:"linear-gradient(135deg,#E1306C,#833AB4,#F77737)",borderRadius:20,padding:"20px",marginBottom:18,textAlign:"center"}}>
            ...
            <div style={{fontSize:17,fontWeight:700,color:"#FFF",marginBottom:4}}>@ozeklaw</div>
            <div style={{fontSize:13,color:"rgba(255,255,255,0.8)",marginBottom:14}}>Follow for immigration tips, legal news & firm updates</div>
            <a href="https://www.instagram.com/ozeklaw/" target="_blank" rel="noreferrer" style={{textDecoration:"none"}}><div style={{background:"rgba(255,255,255,0.2)",border:"1px solid rgba(255,255,255,0.4)",borderRadius:12,padding:"11px 24px",fontSize:14,fontWeight:700,color:"#FFF",display:"inline-block"}}>Open Instagram ↗</div></a>
          </div>
          {[["⚖️","Policy & Case Law Updates","Latest immigration law changes"],["🏢","Business Law Tips","LLC formation, contracts & compliance"],["📋","Form Guides","Common immigration form walkthroughs"],["🌍","Client Success Stories","Real cases and results"],["💡","Know Your Rights","Understand your legal options"]].map(([e,title,desc],i)=>(
            <a key={i} href="https://www.instagram.com/ozeklaw/" target="_blank" rel="noreferrer" style={{display:"block",textDecoration:"none",marginBottom:10}}>
              <div style={{background:"#FFFFFF",borderRadius:18,padding:"14px 16px",display:"flex",gap:14,alignItems:"center",border:"1px solid rgba(0,0,0,0.05)"}}>
                ...
                <div style={{flex:1}}><div style={{fontSize:14,fontWeight:600,color:C.ink,marginBottom:2}}>{title}</div><div style={{fontSize:12,color:C.inkLo}}>{desc}</div></div>
      ...
      {newsTab==="linkedin"&&(
        <div style={{padding:"20px 18px 100px"}}>
          <div style={{background:"linear-gradient(135deg,#0077B5,#00A0DC)",borderRadius:20,padding:"20px",marginBottom:18,textAlign:"center"}}>
            <div style={{fontSize:36,marginBottom:8}}>💼</div>
            <div style={{fontSize:17,fontWeight:700,color:"#FFF",marginBottom:4}}>Ozek Law Firm</div>
            <div style={{fontSize:13,color:"rgba(255,255,255,0.85)",marginBottom:14}}>Professional updates, legal insights & firm news</div>
            <a href="https://www.linkedin.com/company/ozek-law-firm" target="_blank" rel="noreferrer" style={{textDecoration:"none"}}><div style={{background:"rgba(255,255,255,0.2)",border:"1px solid rgba(255,255,255,0.4)",borderRadius:12,padding:"11px 24px",fontSize:14,fontWeight:700,color:"#FFF",display:"inline-block"}}>View on LinkedIn ↗</div></a>
          </div>
  ...
  return(
    <div dir={dir} style={{background:"#F7F5F2",minHeight:"100%"}}>
      <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",padding:"22px 22px 0"}}>
        <div style={{fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:C.gold,fontWeight:600,marginBottom:8}}>IMMIGRATION</div>
        ...
        <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:16,scrollbarWidth:"none"}}>
          {[["tools","🛠️","Tools"],["forms","📝","Forms"],["case","📋","Case Status"],["map","🗺️","Find Offices"]].map(([id,icon,label])=>(
            <button key={id} onClick={()=>setUscisTab(id)} style={{padding:"7px 16px",borderRadius:99,fontSize:13,fontWeight:600,cursor:"pointer",border:"none",background:uscisTab===id?C.gold:"rgba(255,255,255,0.12)",color:uscisTab===id?"#0F1923":"rgba(255,255,255,0.75)",transition:"all 0.2s",flexShrink:0}}>{icon} {label}</button>
          ))}
        </div>
      </div>
      {uscisTab==="forms"&&(
        <div style={{padding:"20px 18px 100px"}}>
          ...
          <div style={{background:"linear-gradient(135deg,#0A1628,#0D1E3A)",borderRadius:18,padding:"16px",border:"1px solid rgba(201,168,76,0.20)",marginBottom:12}}>
            <div style={{display:"flex",gap:12,alignItems:"center"}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:700,color:C.hi,marginBottom:2}}>Not sure which form?</div>
                <div style={{fontSize:12,color:C.md}}>Contact Ozek Law for a consultation</div>
              </div>
              <a href="https://forms.office.com/r/EsAdj00xFP" target="_blank" rel="noreferrer" style={{textDecoration:"none",flexShrink:0}}>
                <div style={{background:C.gold,borderRadius:10,padding:"8px 12px",fontSize:12,fontWeight:700,color:"#0F1923"}}>Fill Out ↗</div>
              </a>
            </div>
          </div>
{[
            ...
            <a key={i} href={f.url} target="_blank" rel="noreferrer" style={{display:"block",textDecoration:"none",marginBottom:12}}>
              <div style={{background:f.color,border:`1px solid ${f.border}`,borderRadius:18,padding:"16px",display:"flex",gap:14,alignItems:"center"}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:10,letterSpacing:1.5,textTransform:"uppercase",color:C.goldDim,fontWeight:600,marginBottom:3}}>{f.cat}</div>
                  <div style={{fontSize:14,fontWeight:700,color:C.ink,marginBottom:2}}>{f.title}</div>
                  <div style={{fontSize:11,color:C.inkLo}}>{f.subtitle}</div>
                </div>
                <div style={{flexShrink:0,background:C.gold,borderRadius:10,padding:"7px 12px",fontSize:12,fontWeight:700,color:"#0F1923"}}>Fill Out ↗</div>
              </div>
            </a>
          ))}
        </div>
        ...
        <div style={{padding:"20px 18px 100px"}}>
          {TOOL_GROUPS.map(g=>(
            <div key={g.label} style={{marginBottom:22}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}><span style={{fontSize:16}}>{g.icon}</span><div style={{fontSize:14,fontWeight:700,color:C.ink}}>{g.label}</div></div>
              {g.items.map(r=>(
                <a key={r.title} href={r.url} target="_blank" rel="noreferrer" style={{display:"block",textDecoration:"none",marginBottom:8}}>
                  <div style={{background:r.color,border:`1px solid ${r.border}`,borderRadius:16,padding:"14px 16px",display:"flex",gap:14,alignItems:"center"}}>
                    ...
                    <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:C.ink,marginBottom:2}}>{r.title}</div><div style={{fontSize:12,color:C.inkLo}}>{r.sub}</div></div>
                    <span style={{color:C.gold,fontSize:17,opacity:0.8}}>↗</span>
                  </div>
                </a>
              ))}
            </div>
      ...
      {uscisTab==="case"&&(
        <div style={{padding:"20px 18px 100px"}}>
          <div style={{background:"#FFFFFF",borderRadius:20,padding:"18px",marginBottom:14,border:"1px solid rgba(0,0,0,0.05)"}}>
            <div style={{fontSize:11,color:C.inkLo,letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>Receipt Number</div>
            <input value={num} onChange={e=>setNum(e.target.value.toUpperCase())} placeholder="e.g. WAC2315151234" style={{width:"100%",background:"rgba(0,0,0,0.04)",border:"1.5px solid rgba(0,0,0,0.08)",borderRadius:13,padding:"13px 16px",color:C.ink,fontSize:16,outline:"none",letterSpacing:1.5,marginBottom:12,fontFamily:"monospace"}}/>
            <GoldBtn full onPress={check} style={{color:"#0F1923"}}>{loading?"Checking…":"Check Status"}</GoldBtn>
            <div style={{textAlign:"center",marginTop:10,fontSize:12,color:C.inkLo}}>Or visit <a href="https://egov.uscis.gov/casestatus/landing.do" target="_blank" rel="noreferrer" style={{color:C.goldDim,fontWeight:600}}>USCIS.gov ↗</a></div>
          </div>
          ...
          {res&&!loading&&(
            <div style={{background:"#FFFFFF",borderRadius:20,padding:"18px",marginBottom:14,border:"1.5px solid rgba(201,168,76,0.25)"}}>
              ...
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:15,color:C.ink,lineHeight:1.55,margin:"12px 0 14px"}}>{res.status}</div>
              <Divider light/>
              {[["Receipt",num],["Form",res.form],["Center",res.center],["Updated",res.updated]].map(([l,v])=>(
                <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"6px 0"}}><span style={{fontSize:12,color:C.inkLo}}>{l}</span><span style={{fontSize:12,fontWeight:600,color:C.ink}}>{v}</span></div>
              ))}
            </div>
          )}
          <div style={{background:"rgba(201,168,76,0.09)",borderRadius:20,padding:"18px",marginBottom:20,border:"1px solid rgba(201,168,76,0.22)"}}>
            <div style={{fontSize:14,fontWeight:700,color:C.ink,marginBottom:10}}>Processing Times</div>
            <div style={{position:"relative",marginBottom:12}}>
              <select value={selForm} onChange={e=>setSelForm(e.target.value)} style={{width:"100%",background:"#FFF",border:"1.5px solid rgba(201,168,76,0.30)",borderRadius:13,padding:"12px 36px 12px 14px",color:C.ink,fontSize:15,outline:"none",cursor:"pointer"}}>
                {USCIS_FORMS.map(f=><option key={f} value={f}>{f}</option>)}
              </select>
              <span style={{position:"absolute",right:13,top:"50%",transform:"translateY(-50%)",color:C.gold,pointerEvents:"none",fontSize:13}}>▾</span>
            </div>
          ...
          <div style={{fontSize:14,fontWeight:700,color:C.ink,marginBottom:12}}>Receipt Prefix Guide</div>
          {[["EAC","Vermont SC"],["LIN","Nebraska SC"],["SRC","Texas SC"],["WAC","California SC"],["NBC","National Benefits Center"],["IOE","Online / Electronic"]].map(([code,name])=>(
            <div key={code} style={{display:"flex",gap:12,alignItems:"center",padding:"11px 0",borderBottom:"1px solid rgba(0,0,0,0.06)"}}>
              <span style={{background:"#0A1628",color:"#FFF",padding:"5px 10px",borderRadius:8,fontSize:11,fontWeight:700,fontFamily:"monospace"}}>{code}</span>
      ...
      {uscisTab==="map"&&(
        <div style={{padding:"20px 18px 100px"}}>
          <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",borderRadius:20,padding:"14px 10px 10px",marginBottom:14,border:"1px solid rgba(201,168,76,0.20)"}}>
            <div style={{fontSize:12,fontWeight:600,color:C.gold,marginBottom:8,textAlign:"center",letterSpacing:1}}>TAP A STATE</div>
            ...
            {selState&&<div style={{textAlign:"center",fontSize:12,color:C.gold,fontWeight:600,marginTop:2,paddingBottom:4}}>{STATE_NAMES[selState]||selState} selected ✓</div>}
          </div>
          <div style={{background:"#FFFFFF",borderRadius:20,padding:"18px",marginBottom:14,border:"1px solid rgba(0,0,0,0.05)"}}>
            <div style={{fontSize:14,fontWeight:700,color:C.ink,marginBottom:4}}>Or search by state</div>
            <div style={{position:"relative"}}>
              <select value={selState||""} onChange={e=>setSelState(e.target.value||null)} style={{width:"100%",background:"#FFF",border:"1.5px solid rgba(201,168,76,0.30)",borderRadius:13,padding:"13px 36px 13px 14px",color:selState?C.ink:C.inkLo,fontSize:15,outline:"none",cursor:"pointer"}}>
          ...
          {selState&&(()=>{const o=STATE_OFFICES[selState]||DEF_OFFICE;return(<div className="pop-in"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><div style={{fontSize:17,fontWeight:700,color:C.ink}}>{STATE_NAMES[selState]||selState}</div><button onClick={()=>setSelState(null)} style={{fontSize:13,color:C.goldDim,fontWeight:600,background:"none",border:"none",cursor:"pointer"}}>← Change</button></div><div style={{background:"rgba(10,132,255,0.08)",border:"1px solid rgba(10,132,255,0.22)",borderRadius:18,padding:"16px",marginBottom:12}}><div style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:8}}><div style={{width:36,height:36,borderRadius:10,background:"rgba(10,132,255,0.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:19,flexShrink:0}}>🏛️</div><div><div style={{fontSize:14,fontWeight:700,color:C.ink}}>USCIS Field Office</div><div style={{fontSize:12,color:C.inkLo}}>Local immigration services</div></div></div><div style={{fontSize:13,color:C.inkMd,lineHeight:1.7,whiteSpace:"pre-line",marginBottom:8}}>{o.uscis}</div><a href="https://www.uscis.gov/about-us/find-a-uscis-office/field-offices" target="_blank" rel="noreferrer" style={{textDecoration:"none"}}><div style={{fontSize:12,color:C.blue,fontWeight:600}}>Find all USCIS offices ↗</div></a></div><div style={{background:"rgba(191,90,242,0.08)",border:"1px solid rgba(191,90,242,0.22)",borderRadius:18,padding:"16px",marginBottom:12}}><div style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:8}}><div style={{width:36,height:36,borderRadius:10,background:"rgba(191,90,242,0.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:19,flexShrink:0}}>⚖️</div><div><div style={{fontSize:14,fontWeight:700,color:C.ink}}>Immigration Court (EOIR)</div><div style={{fontSize:12,color:C.inkLo}}>Hearings & removal proceedings</div></div></div><div style={{fontSize:13,color:C.inkMd,lineHeight:1.7,whiteSpace:"pre-line",marginBottom:8}}>{o.court}</div><a href="https://www.justice.gov/eoir/eoir-immigration-court-listing" target="_blank" rel="noreferrer" style={{textDecoration:"none"}}><div style={{fontSize:12,color:C.purple,fontWeight:600}}>Find all immigration courts ↗</div></a></div><div style={{fontSize:14,fontWeight:700,color:C.ink,margin:"16px 0 10px"}}>Federal Resources</div>{FEDERAL.map(r=>(<a key={r.title} href={r.url} target="_blank" rel="noreferrer" style={{display:"block",textDecoration:"none",marginBottom:8}}><div style={{background:"#FFFFFF",border:"1px solid rgba(0,0,0,0.06)",borderRadius:14,padding:"13px 14px",display:"flex",gap:12,alignItems:"center"}}><div style={{fontSize:20,flexShrink:0}}>{r.icon}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:C.ink}}>{r.title}</div><div style={{fontSize:12,color:C.inkLo}}>{r.sub}</div></div><span style={{color:C.gold,fontSize:15,opacity:0.7}}>↗</span></div></a>))}</div>);})()}
        </div>
      )}
    </div>
  );
  ...
  return(
    <div dir={dir} style={{background:"#F7F5F2",minHeight:"100%"}}>
      <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",padding:"22px 22px 28px"}}>
        <div style={{fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:C.gold,fontWeight:600,marginBottom:8}}>BUSINESS</div>
        {onBack&&<button onClick={onBack} style={{display:"flex",alignItems:"center",gap:6,color:C.gold,fontSize:14,marginBottom:12,fontWeight:600,background:"none",border:"none",cursor:"pointer"}}>‹ Back to News</button>}
        ...
        <p style={{fontSize:13,color:C.md,fontWeight:300}}>{t.regsSub}</p>
      </div>
      <div style={{padding:"20px 18px 100px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div style={{fontSize:15,fontWeight:700,color:C.ink}}>{t.clientAlerts}</div>
        ...
        {CLIENT_ALERTS.map((a,i)=>(
          <div key={i} style={{background:"#FFFFFF",borderRadius:18,padding:"14px 16px",marginBottom:10,border:"1px solid rgba(0,0,0,0.05)",borderLeft:`3px solid ${a.color}`}}>
            <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:7}}>
              ...
              <span style={{fontSize:11,color:C.inkLo,marginLeft:"auto"}}>{a.date}</span>
            </div>
            <div style={{fontSize:13,fontWeight:700,color:C.ink,marginBottom:5}}>{a.title}</div>
            <div style={{fontSize:12,color:C.inkMd,lineHeight:1.55}}>{a.body}</div>
          </div>
        ))}
        <div style={{fontSize:15,fontWeight:700,color:C.ink,margin:"22px 0 12px"}}>{t.practiceAreasTitle||"Practice Area Articles"}</div>
        {PRACTICE_ARTICLES.map((a,i)=>(
          <div key={i} onClick={()=>window.open("https://ozeklaw.com/practiceareas/","_blank")} style={{display:"flex",gap:12,alignItems:"center",background:"#FFFFFF",borderRadius:16,padding:"13px 14px",marginBottom:8,border:"1px solid rgba(0,0,0,0.05)",cursor:"pointer"}}>
            <div style={{width:40,height:40,borderRadius:11,background:"rgba(201,168,76,0.10)",border:"1px solid rgba(201,168,76,0.18)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:19,flexShrink:0}}>{a.emoji}</div>
            <div style={{flex:1}}><Badge bg="rgba(201,168,76,0.10)" color={C.goldDim}>{a.cat}</Badge><div style={{fontSize:13,fontWeight:600,color:C.ink,marginTop:5,lineHeight:1.3}}>{a.title}</div></div>
            <span style={{color:C.gold,fontSize:17,opacity:0.7}}>›</span>
          </div>
        ))}
      </div>
  ...
  return(
    <div dir={dir} style={{background:"#F7F5F2",minHeight:"100%"}}>
      <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",padding:"22px 22px 28px",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse 70% 60% at 80% 20%,rgba(201,168,76,0.10),transparent)"}}/>
        <div style={{position:"relative"}}>
          <div style={{fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:C.gold,fontWeight:600,marginBottom:8}}>OUR FIRM</div>
          ...
          <p style={{fontSize:13,color:C.md,fontWeight:300}}>Reach out directly via WhatsApp</p>
        </div>
      </div>
      <div style={{padding:"20px 18px 100px"}}>
        {TEAM.map(m=>(
          <div key={m.name} style={{background:"#FFFFFF",borderRadius:22,marginBottom:14,border:"1px solid rgba(0,0,0,0.06)",boxShadow:"0 2px 16px rgba(0,0,0,0.07)",overflow:"hidden"}}>
            ...
            <div style={{padding:"14px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
              <div>
                <div style={{fontSize:16,fontWeight:700,color:C.ink,marginBottom:2}}>{m.name}</div>
                <div style={{fontSize:12,color:C.inkLo}}>{m.role}</div>
              </div>
              <button onClick={()=>window.open("https://wa.me/"+m.wa.replace(/[^0-9]/g,"")+"?text=Hello%2C+I+would+like+to+speak+with+"+encodeURIComponent(m.name),"_blank")}
                style={{display:"flex",alignItems:"center",gap:7,background:"#25D366",border:"none",borderRadius:12,padding:"10px 18px",fontSize:14,fontWeight:700,color:"#FFF",cursor:"pointer",boxShadow:"0 3px 12px rgba(37,211,102,0.35)",flexShrink:0}}>
        ...
        <a href="https://ozeklaw.com/our-team/" target="_blank" rel="noreferrer" style={{display:"block",textDecoration:"none",marginTop:8}}>
          <div style={{background:"linear-gradient(135deg,#0A1628,#0D1E3A)",borderRadius:18,padding:"16px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{fontSize:14,fontWeight:600,color:C.hi}}>Full Team & Bio</div>
    ...
    <div dir={dir} style={{minHeight:"100%",background:"linear-gradient(160deg,#080F1E 0%,#0D1628 60%,#111D33 100%)"}}>
      <div style={{padding:"40px 24px 100px"}}>
        <div style={{textAlign:"center",marginBottom:36}}>
          ...
          <p style={{fontSize:14,color:C.md,fontWeight:300}}>{t.portalSub}</p>
        </div>
        <div style={{background:C.blueBg,border:"1px solid rgba(10,132,255,0.20)",borderRadius:16,padding:"13px 16px",marginBottom:22}}>
          <div style={{fontSize:12,fontWeight:700,color:C.blue,marginBottom:4}}>🔗 Powered by Docketwise</div>
          <div style={{fontSize:12,color:"rgba(10,132,255,0.7)",lineHeight:1.5}}>Sign in to access tasks, documents, invoices, and live case updates.</div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:14,marginBottom:18}}>
          <div>
            <div style={{fontSize:11,color:C.lo,letterSpacing:1.2,textTransform:"uppercase",marginBottom:8}}>{t.clientIdLabel}</div>
            <input value={id} onChange={e=>setId(e.target.value)} placeholder={t.clientIdPlaceholder} className="field" style={{fontSize:16}}/>
          </div>
          <div>
            <div style={{fontSize:11,color:C.lo,letterSpacing:1.2,textTransform:"uppercase",marginBottom:8}}>{t.passwordLabel}</div>
            <input type="password" value={pw} onChange={e=>setPw(e.target.value)} placeholder={t.passwordPlaceholder} onKeyDown={e=>e.key==="Enter"&&login()} className="field" style={{fontSize:16}}/>
          </div>
          {err&&<div style={{background:C.redBg,color:C.red,borderRadius:12,padding:"11px 14px",fontSize:13}}>{err}</div>}
          <GoldBtn full onPress={login} style={{padding:"16px",fontSize:16,borderRadius:16,color:"#0F1923"}}>{loading?t.signingIn:t.signIn}</GoldBtn>
        </div>
        <div style={{textAlign:"center",marginBottom:12,fontSize:12,color:C.lo}}>Try: OZ001 · OZ002 · OZ003 · OZ004</div>
        <div style={{textAlign:"center"}}>
          <a href="https://client.docketwise.com" target="_blank" rel="noreferrer" style={{fontSize:13,color:C.gold,fontWeight:600,textDecoration:"none",opacity:0.8}}>→ Go directly to Docketwise ↗</a>
        </div>
      </div>
    </div>
  );
    ...
    <div dir={dir} style={{background:"#F7F5F2",minHeight:"100%"}}>
      <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",padding:"20px 22px 26px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18}}>
          <div>
            <div style={{fontSize:13,color:C.lo,marginBottom:2,fontWeight:300}}>{t.goodMorning}</div>
        ...
        <div style={{background:"rgba(255,255,255,0.08)",backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",border:"1px solid rgba(255,255,255,0.14)",borderRadius:16,padding:"14px 16px"}}>
          <div style={{fontSize:10,color:C.gold,letterSpacing:1.5,textTransform:"uppercase",marginBottom:5}}>{t.nextAction}</div>
          <div style={{fontSize:14,fontWeight:600,color:C.hi}}>{client.next}</div>
          <div style={{fontSize:12,color:C.lo,marginTop:3}}>{client.caseNum} · {client.atty}</div>
        </div>
      </div>
      <div style={{padding:"18px 18px"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
          {[{label:t.openTasks,val:pending,color:C.gold,icon:"✅",tab:"tasks"},{label:t.amountDue,val:`$${due.toLocaleString()}`,color:C.red,icon:"💳",tab:"billing"},{label:t.matters,val:client.matters.length,color:C.blue,icon:"⚖️",tab:null},{label:t.newDocs,val:"3",color:C.green,icon:"📁",tab:"docs"}].map(s=>(
            <div key={s.label} onClick={s.tab?()=>setTab(s.tab):undefined} style={{background:"#FFFFFF",borderRadius:20,padding:"16px",border:"1px solid rgba(0,0,0,0.05)",cursor:s.tab?"pointer":"default",borderTop:`3px solid ${s.color}`}}>
              ...
              <div style={{fontSize:11,color:C.inkLo,marginBottom:4}}>{s.label}</div>
              <div style={{fontSize:22,fontWeight:800,color:s.color,fontFamily:"'Playfair Display',serif"}}>{s.val}</div>
            </div>
          ))}
        </div>
        <div style={{fontSize:14,fontWeight:700,color:C.ink,marginBottom:10}}>{t.quickActions}</div>
        <div style={{display:"flex",gap:8,marginBottom:22}}>
          {[["📤",t.upload,()=>setTab("docs")],["💬",t.askAtty,()=>setTab("chat")],["📋",t.formsLabel,()=>setTab("forms")],["🔗",t.docketwise,()=>window.open("https://client.docketwise.com","_blank")]].map(([icon,label,fn])=>(
            <button key={label} onClick={fn} className="tap" style={{flex:1,background:"#FFFFFF",border:"1px solid rgba(0,0,0,0.06)",borderRadius:16,padding:"13px 4px",display:"flex",flexDirection:"column",alignItems:"center",gap:5,cursor:"pointer"}}>
              ...
              <span style={{fontSize:10,fontWeight:600,color:C.inkLo}}>{label}</span>
            </button>
          ))}
        </div>
        <div style={{fontSize:14,fontWeight:700,color:C.ink,marginBottom:10}}>{t.yourMatters}</div>
        {client.matters.map(m=>(
          <div key={m} style={{display:"flex",gap:12,alignItems:"center",background:"#FFFFFF",borderRadius:16,padding:"13px 14px",marginBottom:9,border:"1px solid rgba(0,0,0,0.05)"}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:14,fontWeight:600,color:C.ink}}>{m}</div><div style={{fontSize:12,color:C.inkLo,marginTop:1}}>{client.atty}</div></div>
        ...
        <button onClick={()=>setShowBooking(true)} style={{width:"100%",display:"flex",gap:12,alignItems:"center",background:"linear-gradient(135deg,#0A1628,#0D1E3A)",borderRadius:18,padding:"14px 16px",border:"1px solid rgba(201,168,76,0.20)",marginBottom:14,cursor:"pointer"}}>
          <div style={{width:38,height:38,borderRadius:11,background:"rgba(201,168,76,0.18)",border:"1px solid rgba(201,168,76,0.28)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:19,flexShrink:0}}>📅</div>
          <div style={{flex:1,textAlign:"left"}}><div style={{fontSize:14,fontWeight:700,color:"#FFF",marginBottom:1}}>Schedule a Consultation</div><div style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}>Book directly with Atty. Tolga Ozek</div></div>
          <span style={{color:"#C9A84C",fontSize:17,opacity:0.7}}>›</span>
        </button>
        <div style={{background:"rgba(201,168,76,0.09)",borderRadius:18,padding:"14px 16px",border:"1px solid rgba(201,168,76,0.20)"}}>
          <div style={{display:"flex",gap:12,alignItems:"center"}}>
            <div style={{fontSize:22}}>🔗</div>
            <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:C.ink,marginBottom:1}}>{t.syncedWith}</div><div style={{fontSize:12,color:C.inkLo}}>{t.syncedSub}</div></div>
    ...
    <div dir={dir} style={{background:"#F7F5F2",minHeight:"100%"}}>
      <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",padding:"22px 22px 28px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
          <div><div style={{fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:C.gold,fontWeight:600,marginBottom:8}}>CASE MANAGEMENT</div><h1 style={{fontFamily:"'Playfair Display',serif",fontSize:24,fontWeight:700,color:C.hi,letterSpacing:-0.4}}>{t.myTasks}</h1></div>
          ...
          return(
            <div key={task.id} className="tap" style={{background:"#FFFFFF",borderRadius:20,padding:"16px",marginBottom:10,border:"1px solid rgba(0,0,0,0.05)",opacity:task.status==="Done"?0.5:1}}>
              <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                <div style={{width:36,height:36,borderRadius:11,background:sc.bg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>
                  {task.status==="Done"?"✓":task.status==="Progress"?"⏳":task.status==="Scheduled"?"📅":"○"}
                </div>
                <div style={{flex:1}}>
                  <div style={{fontSize:14,fontWeight:600,color:C.ink,textDecoration:task.status==="Done"?"line-through":"none",marginBottom:4}}>{task.title}</div>
                  <div style={{fontSize:12,color:C.inkLo,marginBottom:8}}>Due {task.due}</div>
  ...
  return(
    <div dir={dir} style={{background:"#F7F5F2",minHeight:"100%"}}>
      <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",padding:"78px 22px 28px"}}>
        ...
        <p style={{fontSize:13,color:C.md,fontWeight:300,marginTop:4}}>{t.docsSub}</p>
      </div>
      <div style={{padding:"18px 18px 100px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
          <button onClick={()=>fileRef.current.click()} style={{background:"#FFFFFF",border:"2px dashed rgba(201,168,76,0.40)",borderRadius:18,padding:"20px 10px",display:"flex",flexDirection:"column",alignItems:"center",gap:7,cursor:"pointer"}}>
            <span style={{fontSize:26}}>📤</span><span style={{fontSize:13,fontWeight:600,color:C.ink}}>{t.uploadFile}</span><span style={{fontSize:11,color:C.inkLo}}>{t.pdfTypes}</span>
          </button>
          <button style={{background:"#FFFFFF",border:"1px solid rgba(0,0,0,0.07)",borderRadius:18,padding:"20px 10px",display:"flex",flexDirection:"column",alignItems:"center",gap:7,cursor:"pointer"}}>
            <span style={{fontSize:26}}>📷</span><span style={{fontSize:13,fontWeight:600,color:C.ink}}>{t.scanDoc}</span>
            <span style={{fontSize:11,color:C.inkLo}}>{t.useCamera}</span>
          </button>
        </div>
        <div style={{fontSize:14,fontWeight:700,color:C.ink,marginBottom:10}}>{t.allFiles}</div>
        {[{name:"Passport_Copy.pdf",size:"1.2MB",date:"Mar 1",status:"Verified"},{name:"Birth_Certificate_EN.pdf",size:"456KB",date:"Feb 28",status:"Verified"},{name:"I-94_Record.pdf",size:"89KB",date:"Feb 20",status:"Pending"},...uploaded].map((d,i)=>{const sc=stC(d.status);return(
          <div key={i} style={{display:"flex",gap:12,alignItems:"center",background:"#FFFFFF",borderRadius:16,padding:"13px 14px",marginBottom:8,border:"1px solid rgba(0,0,0,0.05)"}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:C.ink,marginBottom:2}}>{d.name}</div><div style={{fontSize:11,color:C.inkLo}}>{d.size} · {d.date}</div></div>
...
function FormsPage({t,lang}){
  ...
  if(done)return(<div dir={dir} style={{padding:"70px 24px",textAlign:"center",background:"#F7F5F2",minHeight:"100%"}}><div style={{fontSize:56,marginBottom:16}}>✅</div><h2 style={{fontFamily:"'Playfair Display',serif",fontSize:24,color:C.ink,marginBottom:8}}>{t.submitted}</h2><p style={{fontSize:15,color:C.inkMd,marginBottom:28,lineHeight:1.6}}>{t.submittedSub}</p><GoldBtn onPress={()=>{setDone(false);setSel(null);setData({})}} style={{color:"#0F1923"}}>{t.submitAnother}</GoldBtn></div>);
  if(sel)return(
    <div dir={dir} style={{background:"#F7F5F2",minHeight:"100%"}}>
      <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",padding:"78px 22px 28px"}}>
        <button onClick={()=>{setSel(null);setData({})}} style={{display:"flex",alignItems:"center",gap:8,color:C.gold,fontSize:15,marginBottom:16,fontWeight:700,background:"rgba(201,168,76,0.10)",border:"1px solid rgba(201,168,76,0.25)",borderRadius:10,padding:"8px 14px",cursor:"pointer"}}>‹ {t.back}</button>
        <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:22,fontWeight:700,color:C.hi}}>{sel.title}</h2>
        <p style={{fontSize:13,color:C.md,marginTop:4}}>{sel.sub}</p>
      </div>
      <div style={{padding:"20px 18px 100px"}}>
        {sel.fields.map(f=>(
          <div key={f} style={{marginBottom:14}}>
            <div style={{fontSize:11,color:C.inkLo,letterSpacing:1,textTransform:"uppercase",marginBottom:7}}>{f}</div>
            <input value={data[f]||""} onChange={e=>setData(p=>({...p,[f]:e.target.value}))} style={{width:"100%",background:"#FFFFFF",border:"1.5px solid rgba(0,0,0,0.08)",borderRadius:14,padding:"13px 16px",color:C.ink,fontSize:15,outline:"none"}}/>
          </div>
        ))}
  ...
  return(
    <div dir={dir} style={{background:"#F7F5F2",minHeight:"100%"}}>
      <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",padding:"78px 22px 28px"}}>
        ...
        <p style={{fontSize:13,color:C.md,marginTop:4}}>{t.formsSub}</p>
      </div>
      <div style={{padding:"18px 18px 100px"}}>
        {forms.map(f=>(
          <div key={f.id} onClick={()=>setSel(f)} style={{display:"flex",gap:12,alignItems:"center",background:"#FFFFFF",borderRadius:18,padding:"16px",marginBottom:10,border:"1px solid rgba(0,0,0,0.05)",cursor:"pointer"}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:15,fontWeight:700,color:C.ink,marginBottom:2}}>{f.title}</div><div style={{fontSize:12,color:C.inkLo}}>{f.sub}</div></div>
            <span style={{color:C.gold,fontSize:19,opacity:0.7}}>›</span>
          </div>
        ))}
      </div>
  ...
  return(
    <div dir={dir} style={{display:"flex",flexDirection:"column",height:"calc(100vh - 60px)",background:"#F7F5F2"}}>
      <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",padding:"78px 22px 20px",flexShrink:0}}>
        <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:19,fontWeight:700,color:C.hi,marginBottom:2}}>{t.askAttorney}</h1>
        <p style={{fontSize:12,color:C.md,fontWeight:300}}>{t.askSub}</p>
      </div>
      <div style={{flex:1,overflow:"auto",padding:"14px 16px",display:"flex",flexDirection:"column",gap:10}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
            <div style={{maxWidth:"80%",padding:"11px 15px",borderRadius:18,fontSize:14,lineHeight:1.6,whiteSpace:"pre-wrap",background:m.role==="user"?C.gold:"#FFFFFF",color:m.role==="user"?"#0F1923":C.ink,boxShadow:m.role==="user"?`0 4px 16px ${C.goldGlow}`:"0 1px 8px rgba(0,0,0,0.07)"}}>{m.content}</div>
          </div>
        ))}
        {sent&&<div style={{display:"flex",justifyContent:"flex-end"}}><div style={{background:C.gold,padding:"8px 14px",borderRadius:16,fontSize:13,color:"#0F1923",opacity:0.6}}>Sending…</div></div>}
        <div ref={bottomRef}/>
      </div>
      <div style={{padding:"10px 16px 14px",background:"#F7F5F2",borderTop:"1px solid rgba(0,0,0,0.07)",flexShrink:0}}>
        <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:8}}>
          {t.quickQ.map(q=><button key={q} onClick={()=>setInput(q)} style={{flexShrink:0,background:"#FFFFFF",border:"1px solid rgba(0,0,0,0.08)",borderRadius:99,padding:"7px 13px",fontSize:11,color:C.inkMd,cursor:"pointer",whiteSpace:"nowrap"}}>{q}</button>)}
        </div>
        <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder={t.askPlaceholder} style={{flex:1,background:"#FFFFFF",border:"1.5px solid rgba(0,0,0,0.09)",borderRadius:16,padding:"13px 16px",color:C.ink,fontSize:14,outline:"none"}}/>
          <button onClick={send} style={{width:46,height:46,borderRadius:14,background:C.gold,color:"#0F1923",border:"none",cursor:"pointer",fontSize:19,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>↑</button>
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div dir={dir} style={{background:"#F7F5F2",minHeight:"100%"}}>
      <div style={{background:"linear-gradient(145deg,#0A1628,#0D1E3A)",padding:"78px 22px 28px",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse 70% 60% at 80% 20%,rgba(201,168,76,0.10),transparent)"}}/>
        <div style={{position:"relative"}}>
          <div style={{fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:C.gold,fontWeight:600,marginBottom:8}}>ACCOUNT</div>
          <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:24,fontWeight:700,color:C.hi,letterSpacing:-0.4}}>{t.billing}</h1>
        </div>
      </div>
      <div style={{padding:"18px 18px 100px"}}>
        <div style={{background:"#FFFFFF",borderRadius:20,padding:"18px",marginBottom:14,border:"1px solid rgba(0,0,0,0.05)"}}>
          <div style={{fontSize:10,letterSpacing:2,textTransform:"uppercase",color:C.inkLo,marginBottom:12}}>BILLING OVERVIEW</div>
          <div style={{display:"flex",gap:10,marginBottom:8}}>
            {[["#30D158","Paid",paid],["#FF453A","Due",due],["#0A84FF","Upcoming",soon]].map(([color,label,amt])=>(
              <div key={label} style={{flex:1,background:"rgba(0,0,0,0.03)",borderRadius:14,padding:"12px",borderTop:`3px solid ${color}`}}>
                <div style={{fontSize:11,color:C.inkLo,marginBottom:3}}>{label}</div>
                <div style={{fontSize:19,fontWeight:800,color:C.ink,fontFamily:"'Playfair Display',serif"}}>${amt.toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
        {due>0&&(
          <div style={{background:C.redBg,border:"1px solid rgba(255,69,58,0.20)",borderRadius:18,padding:"14px 16px",marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}>
              <div><div style={{fontSize:14,fontWeight:700,color:C.red,marginBottom:2}}>{t.paymentRequired}</div><div style={{fontSize:12,color:C.inkLo}}>billing@ozeklaw.com</div></div>
        ...
        <div style={{fontSize:14,fontWeight:700,color:C.ink,marginBottom:10}}>{t.invoices}</div>
        {BILLS.map((b,i)=>{const sc=stC[b.status]||{bg:"rgba(0,0,0,0.05)",c:C.inkLo};return(
          <div key={i} style={{background:"#FFFFFF",borderRadius:18,padding:"16px",marginBottom:10,border:"1px solid rgba(0,0,0,0.05)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}>
              <div><div style={{fontSize:14,fontWeight:700,color:C.ink}}>{b.id}</div><div style={{fontSize:11,color:C.inkLo,marginTop:1}}>{b.date}</div></div>
              <Badge bg={sc.bg} color={sc.c}>{b.status}</Badge>
            </div>
            <div style={{fontSize:13,color:C.inkMd,marginBottom:10}}>{b.desc}</div>
...
function SplashScreen({onDone}){
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:99999,background:"#080F1E",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",animation:"fogOut 0.5s ease 2.3s both"}}>
      ...
      <div style={{marginTop:14,fontSize:11,letterSpacing:3.5,textTransform:"uppercase",color:"rgba(201,168,76,0.65)",fontWeight:500,position:"relative",zIndex:1,animation:"fogIn 1.8s cubic-bezier(0.22,1,0.36,1) 0.7s both"}}>
        ...
        {client&&isClient&&(
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"5px 20px",background:"rgba(8,15,30,0.75)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,0.06)",flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:12,color:C.lo}}>{client.name} · {client.caseNum}</span>
            </div>
